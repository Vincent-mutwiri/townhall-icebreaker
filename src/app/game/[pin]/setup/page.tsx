...
export default function GameSetupPage({ params }: { params: Promise<{ pin: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [globalRes, gameRes] = await Promise.all([
        fetch('/api/questions'),
            ...
            <div className="pt-8 text-center space-y-4">
              {gameQuestions.length > 0 && (
                <p className="text-sm text-muted-foreground">
                  Ready to start! Your game has {gameQuestions.length} question{gameQuestions.length !== 1 ? 's' : ''}.
                </p>
              )}
              <Button 
                size="lg" 
              ...
              >
                {gameQuestions.length === 0 ? "Add Questions to Start" : "Start Game Now!"}
              </Button>
            </div>
          </CardContent>
        </Card>
